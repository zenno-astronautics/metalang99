...
Macrolop is an embedded [metalanguage] that facilitates [language-oriented programming] in plain C -- instead of solving problems using overly liberal mechanisms of C, solve them via your own [domain-specific languages]. Macrolop accomplishes its goal by extending the standard preprocessor with high-level programming constructs to let us deal with [metaprogramming] on a large scale.
