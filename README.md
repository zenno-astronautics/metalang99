...
To solve the problem, I have implemented Epilepsy -- a functional programming language executing on any standard-confirming preprocessor. Most importantly, it is designed in such a way to permit macro recursion so typically you will not find yourself in a sutiation where expansion accidentally gets [blocked by some mysterious reason](https://github.com/pfultz2/Cloak/wiki/C-Preprocessor-tricks,-tips,-and-idioms#recursion). It also exports a bouquet of nice features like partial application, error reporting, and all the stuff.
