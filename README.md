...
To solve the problem, I have implemented Epilepsy -- a functional programming language executing on any standard-confirming preprocessor. Most importantly, it is designed in such a way to permit macro recursion so typically you will not find yourself in a sutiation where some macro accidentally gets [blueprinted] by some mysterious reason. It also exports a bouquet of nice features like partial application, error reporting, and all the stuff.
