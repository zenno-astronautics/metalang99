...
Here, `Tree` is a [sum type] -- an extension to the C type system allowing to conveniently encode and deal with alternative data representations. Despite the fact that datatype99 is not a trivial metaprogram, its implementation is straightforward and the interface is type-safe -- thanks to Epilepsy.
